...
export default function BruteForceTree({ step }: { step: Step }) {
  ...
  if (!nodes.length) return <div className="text-[#7C6A5D]">鏆傛棤閫掑綊鏍?/div>;
  ...
  const dfs = (id: string, depth: number) => {
    ...
    const currentY = cursor;
    cursor += 1;
    const n = byId.get(id);
    if (n) ordered.push({ ...n, depth, yOrder: currentY });
    sorted.forEach((s) => dfs(s.id, depth + 1));
  };
  dfs(root, 0);

  const currentLabel = `T(${step.currentState.eggs},${step.currentState.floors})`;
  const matched = ordered.filter((n) => n.label === currentLabel);\n  const focusCandidate = matched.length ? matched[matched.length - 1] : undefined;
  const focusId = focusCandidate?.id ?? (ordered.length ? ordered[ordered.length - 1].id : root);
  ...
  ordered.forEach((n) => {
    pos.set(n.id, { x: leftPad + n.depth * xGap, y: topPad + n.yOrder * yGap });
  });
  ...
  return (
    <div>
      <div className="flex items-center justify-between mb-2">
        <div className="text-sm text-[#7C6A5D]">閫掑綊鏍戯紙灞傜骇甯冨眬锛宐roken 涓?/ safe 涓嬶級</div>
        <div className="text-xs px-2 py-1 rounded-full bg-[#FFF8F0] border border-[#E7D9C8] text-[#8C6E58]">鑱氱劍褰撳墠璺緞</div>
      </div>
      <div className="overflow-auto rounded-2xl border border-[#E7D9C8] bg-[#FFFDF9] p-2">
        <svg width={width} height={height}>
          {edges.map((e, i) => {
            ...
            return (
              <g key={`${e.from}-${e.to}-${i}`} opacity={active ? 1 : 0.25}>
                ...
                <rect x={midX - 26} y={midY - 10} width={52} height={18} rx={9} fill={e.label === 'broken' ? '#FDE6CD' : '#EAF4EA'} stroke={e.label === 'broken' ? '#E5B176' : '#9FBCA3'} />
          ...
          {ordered.map((n) => {
            ...
            return (
              <motion.g key={n.id} initial={{ opacity: 0, scale: 0.9 }} animate={{ opacity: onPath ? 1 : 0.42, scale: isFocus ? 1.04 : 1 }} transition={{ duration: 0.22 }}>
                <rect x={p.x - 60} y={p.y - 22} width={120} height={44} rx={12} fill={style.fill} stroke={style.stroke} strokeWidth={isFocus ? 2.8 : 1.6} style={isFocus ? { filter: 'drop-shadow(0 0 8px rgba(217,119,6,0.35))' } : {}} />
                <text x={p.x} y={p.y + 3} textAnchor="middle" fontSize="12" fill="#4B3A2F" fontWeight={600}>{n.label}</text>
                {isRepeat && <g><rect x={p.x + 26} y={p.y - 30} width={32} height={14} rx={7} fill="#E8B4B8" /><text x={p.x + 42} y={p.y - 20} textAnchor="middle" fontSize="9" fill="#6F3F43">閲嶅</text></g>}
              </motion.g>
            );
          })}
        </svg>
